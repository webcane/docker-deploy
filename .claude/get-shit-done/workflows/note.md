...
- **Global scope**: `/Users/mniedre/git/docker-deploy/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/mniedre/git/docker-deploy/.claude/notes/`)
...
2. Glob `/Users/mniedre/git/docker-deploy/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/mniedre/git/docker-deploy/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/mniedre/git/docker-deploy/.claude/notes/` — works in any directory
