...
```bash
curl -fsSL https://raw.githubusercontent.com/webcane/docker-deploy/v0.8.5/install.sh | sh
```

Or to select a specific version at install time:

```bash
curl -fsSL https://raw.githubusercontent.com/webcane/docker-deploy/master/install.sh | INSTALL_VERSION=v0.8.5 sh
```
