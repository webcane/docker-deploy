...
expected: |
  Kill any running server/service. Clear ephemeral state. Build the binary from scratch with `go build ./cmd/docker-deploy/`. Binary compiles without errors. Running `./docker-deploy --help` (or `docker deploy --help` if installed) returns a help screen without crashing.
